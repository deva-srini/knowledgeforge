...
Benchmarks on NVIDIA A40 (46 GB VRAM, CUDA 12.4):

| Stage | Document | Time |
|---|---|---|
| Docling converter init (cold start) | — | ~95 s (one-time) |
| Parse (cold, first document) | 7-page PDF | ~16 s |
| Parse (warm) | 7-page PDF | ~4.5 s |
| Extract | 54 items | ~18 ms |
| Transform | 54 items | ~88 ms |
| Chunk | 35 chunks from 54 items | ~8 ms |
| Embed | 35 chunks, 384-dim | ~8.3 s |
| Index (ChromaDB upsert) | 35 chunks | ~0.28 s |

**End-to-end demo — `bgf_factsheet.pdf` via `fund_factsheet` workflow:**

```
Parse:     4 pages, 3373 tokens                          [114.9 s]
Extract:   98 items (84 text, 4 table, 10 image)         [0.01 s]
Transform: 98 items → 12,394 char markdown               [0.02 s]
Chunk:     33 chunks, 3049 tokens                        [0.01 s]
Embed:     33 × 384-dim vectors                          [2.94 s]
Index:     33 chunks → fund_factsheets collection        [0.28 s]
─────────────────────────────────────────────────────────────────
Total:                                                  [118.14 s]
```

> The 114 s parse time includes cold-start model loading. Subsequent documents in the same session parse in ~4–5 s.
...
|---|---|---|
| 11 | Metrics Collection & Reporting | API + CLI endpoint exposing per-document and per-stage processing statistics from SQLite |
| 11 | LangSmith Observability | Full tracing of LLM calls, token usage, and prompt versioning via LangSmith |
| 11 | **Docling Agent (Parsing)** | An LLM agent that reviews the Docling-parsed document structure before markdown conversion — correcting misclassified elements (e.g. text blocks mistaken for lists, broken table boundaries, incorrect header levels) to produce accurate, clean structured markdown, especially for complex multi-column PDFs |
| 12 | FastAPI Route Implementations | Complete all document endpoints (process, reprocess, status, list) |
| 13 | End-to-End Integration Test | Full pipeline verification test covering ingest → index round-trip |
